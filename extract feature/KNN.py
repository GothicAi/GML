...
def save_KNN_img(attribute, object_attr_vector, index):
    K = 100
    data = load_label()
    negative, positive = get_corresponding_attr_dataset(data, attribute)
    img_names = os.listdir("./lfw//" + object_attr_vector[0][0].replace(" ", "_"))
    Image.open("./lfw//" + object_attr_vector[0][0].replace(" ", "_") + "//" + img_names[0]).save("./input_img//" + str(index) + ".jpg")
    ...
    for i in range(K):
        try:
            ...
            img_num = neg[i].num[0]
            Image.open("./lfw//lfw//" + filename + "//" + filename + "_" + "%04d" % (img_num) + ".jpg").save("KNNdata/Senior/source" + str(index) + "//" + str(i) + ".jpg")
            filename = pos[i].name[0].replace(" ", "_")
            img_num = pos[i].num[0]
            Image.open("./lfw//lfw//" + filename + "//" + filename + "_" + "%04d" %(img_num) + ".jpg").save("KNNdata/Senior/target" + str(index) + "//" + str(i) + ".jpg")
        except TypeError:
            #print("name_error")
            gg = 1
    if gg:
...
def do(n, step):
    ...
    for i in range(step):
        #print(n+i)
        save_KNN_img(target_attr, data[n+i, :], n+i)
