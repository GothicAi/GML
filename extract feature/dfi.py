...
class DFI:
    def __init__(self):
        self.x = tf.placeholder(tf.float32, [1, 256, 256, 3])
        self.w = tf.placeholder(tf.float32, [1, None])
        self.z = tf.get_variable("z", [1, 256, 256, 3], initializer=tf.random_normal_initializer(stddev=0.02))
    ...
    def train(self):
        ...
        for name in os.listdir("source"):
            w = self.attribute_vector("./source//"+name, "./target//"+name)#Calculate the attribute vector w.
        '''
        H = input_img.shape[0]
        W = input_img.shape[1]
    ...
    def attribute_vector(self, source_name, target_name):
        ...
        for filename in source_filenames:
            ...
            h = img.shape[0]
            w = img.shape[1]
            img = misc.imresize(img, [256, 256])[np.newaxis, :, :, :]
        ...
        np.savez('npz/source_'+source_name[-10:-4]+'.npz', phi_s_bar)
        np.savez('npz/target_'+target_name[-10:-4]+'.npz', phi_t_bar)
